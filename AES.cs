...
namespace SecurityLibrary.AES
{
    ...
    public class AES : CryptographicTechnique
    {
        ...
        string[,] mixColumns(string[,] plain)
        {
        ...
        {"03", "01", "01", "02"}
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    for (int k = 0; k < 4; k++)
                    {
                        string binary = convertToBinary(plain[k, j]);
        ...
        string[,] addRoundKey(string[,] plain, string[,] key)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    result[j, i] = XOR(convertToBinary(plain[j, i]), convertToBinary(key[j, i]));
                }
            }
